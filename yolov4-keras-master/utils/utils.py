...
def get_random_data_with_Mosaic(annotation_line, input_shape, max_boxes=50, hue=.1, sat=1.5, val=1.5):
    ...
    for line in annotation_line:
        ...
        # size of images
        iw, ih = image.size
        # save locations of bounding box
        ...
        # Place the pictures
        # corresponding to the positions of the four divided pictures
        dx = place_x[index]
        dy = place_y[index]
...
def cosine_decay_with_warmup(global_step,
    ...
    if total_steps < warmup_steps:
        raise ValueError('total_steps must be larger or equal to '
                         'warmup_steps.')
    # Set the minimum value of learning rate as 0
    learning_rate = 0.5 * learning_rate_base * (1 + np.cos(np.pi *
                                                           (global_step - warmup_steps - hold_base_rate_steps) / float(
        total_steps - warmup_steps - hold_base_rate_steps)))
    # If hold_base_rate_steps are greater than 0, it indicates that the learning rate remains unchanged for a certain
    # number of steps after warm up
    ...
    if warmup_steps > 0:
        ...
        # Only when the global_step is still in the warm up phase, the linearly increasing learning rate warmup_rate
        # will be used, otherwise the learning rate learning_rate of cosine annealing will be used
...
class WarmUpCosineDecayScheduler(keras.callbacks.Callback):
    """
    Callback，Realize the scheduling of learning rate
    """
    ...
    def on_batch_begin(self, batch, logs=None):
        # The parameter is re-updated every time the lowest point is reached
